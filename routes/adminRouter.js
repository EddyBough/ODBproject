...
adminRouter.post("/ModifyForm/:id", adminGuard, upload.single('photo'), async (req, res) => { // ici en cliquant sur valider dans le form de modificationCustomer, on va post les validations sur mongoDB sur l'id (l'utilisateur donc).
    try {
        ...
        await prestationModel.updateOne(({ _id: req.params.id }), req.body)//Cette ligne utilise la méthode "updateOne" pour mettre à jour un document de la collection "prestations" avec l'ID correspondant à "req.params.id". Elle passe l'objet "req.body" comme argument pour définir les nouvelles valeurs des champs de la collection. Cette méthode est asynchrone, donc nous utilisons le mot clé "await" pour attendre sa résolution avant de continuer.
...
// fonction qui permet de supprimer la prestation ( le form qui récupère ce qu'il y a à delete est sur la page DeleteForm )
